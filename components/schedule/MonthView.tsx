    ...
    <div className="flex h-full flex-col">
      <div className="grid shrink-0 grid-cols-7 border-b border-line bg-surface">
        {WEEKDAYS.map((label, index) => (
          <div
            key={`${label}-${index}`}
            className="py-1.5 text-center text-xs font-extrabold text-muted"
          >
            {label}
          </div>
        ))}
      ...
      <div className="grid min-h-0 flex-1 grid-cols-7 grid-rows-6">
        {days.map((day) => {
          ...
          return (
            ...
            >
              <div
                className={`mb-0.5 text-[11px] font-extrabold ${
                  isToday(day)
                    ? "text-accent"
                    : outside
                      ? "text-muted/60"
              ...
              <div className="flex flex-col gap-0.5">
                {dayJobs.slice(0, 3).map((job) => (
                  <button
                    ...
                    className="block h-3.5 w-full truncate rounded-sm px-1 text-left text-[9px] font-extrabold text-black/80"
                  >
